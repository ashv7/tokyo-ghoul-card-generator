...
import { Coffee, Award, User, MapPin, Heart, Package, Shield } from 'lucide-react'

function CardAnteiku({ cardData }) {
  const qrRef = useRef(null)
  const barcodeRef = useRef(null)

  useEffect(() => {
    if (cardData && qrRef.current) {
      const theme = cardData.theme || { colors: { primary: '#8b4513', background: '#2c1810' } }
  ...
  const theme = cardData.theme || { colors: { primary: '#8b4513', secondary: '#d2691e', background: '#2c1810' } }
      ...
      >
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-400">
                Café Anteiku - 20ème Arrondissement
              </p>
            </div>
            ...
            <div className="flex-1">
              ...
              <div className="space-y-1">
                <div className="flex items-center space-x-2 text-sm">
                  <Coffee size={16} style={{ color: themeColors.primary }} />
                  <span className="text-gray-300">{cardData.role}</span>
                </div>
                <div className="flex items-center space-x-2 text-sm">
                  <Award size={16} style={{ color: themeColors.primary }} />
                  <span className="text-gray-300">{cardData.specialization}</span>
                </div>
                <div className="flex items-center space-x-2 text-sm">
                ...
                {cardData.area && (
                  <div className="flex items-center space-x-2 text-sm">
                    ...
                    <span className="text-gray-300">{cardData.area.location}</span>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="p-6 bg-ghoul-dark/50">
          ...
          <div className="mt-4 pt-4 border-t border-gray-700">
            <div className="flex items-center justify-between">
              <span className="text-sm text-gray-400">Niveau d'expérience</span>
        ...
        {cardData.equipment && (
          <div className="p-6 bg-ghoul-dark/30">
            ...
            <div className="space-y-2 text-xs">
              {cardData.equipment.workItem && (
                <div className="flex items-center justify-between">
                  <span className="text-gray-400">Travail:</span>
                  <span className="text-gray-300">{cardData.equipment.workItem.name}</span>
                </div>
              )}
              {cardData.equipment.protectiveGear && (
                <div className="flex items-center justify-between">
                  <span className="text-gray-400">Protection:</span>
                  <span className="text-gray-300">{cardData.equipment.protectiveGear.name}</span>
                </div>
              )}
