...
import { Shield, Swords, Award, User, MapPin, Heart, Package } from 'lucide-react'

function Card({ cardData, showBack = false }) {
      ...
      >
        ...
        <div className="p-6 bg-ghoul-gray/50">
          <div className="flex items-start space-x-4">
            <div className="w-32 h-32 bg-ghoul-dark rounded-lg border-2 border-ghoul-red flex items-center justify-center overflow-hidden">
              {cardData.photo ? (
            ...
            <div className="flex-1">
              ...
              <div className="space-y-1">
                <div className="flex items-center space-x-2 text-sm">
                  ...
                  <span className="text-gray-300">{cardData.rank}</span>
                </div>
                <div className="flex items-center space-x-2 text-sm">
                  <Award size={16} style={{ color: themeColors.primary }} />
                  <span className="text-gray-300">Type: {cardData.inspectorType}</span>
                </div>
                <div className="flex items-center space-x-2 text-sm">
                  <Swords size={16} style={{ color: themeColors.primary }} />
                  <span className="text-gray-300">{cardData.weapon}</span>
                </div>
                ...
                {cardData.division && (
                  <div className="flex items-center space-x-2 text-sm">
                    ...
                    <span className="text-gray-300">{cardData.division.name}</span>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="p-6 bg-ghoul-dark/50">
          <h4 className="text-sm font-bold mb-3 uppercase" style={{ color: themeColors.primary }}>Compétences dominantes</h4>
          <div className="space-y-2">
            {cardData.dominantSkills.map((skill, index) => (
              <div key={index} className="flex items-center justify-between bg-ghoul-dark/50 px-3 py-2 rounded">
                ...
                <span className="text-sm font-bold" style={{ color: themeColors.primary }}>{cardData.scores[skill]}/6</span>
              </div>
            ))}
          </div>
          ...
          <div className="mt-4 pt-4 border-t border-gray-700">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xl font-bold" style={{ color: themeColors.primary }}>{cardData.totalScore}</span>
            </div>
          </div>
        </div>

        {/* Equipment Section */}
        {cardData.secondaryEquipment && (
          <div className="p-6 bg-ghoul-dark/30">
            ...
            <div className="space-y-2 text-xs">
              {cardData.secondaryEquipment.armor && (
                <div className="flex items-center justify-between">
                  <span className="text-gray-400">Armure:</span>
                  <span className="text-gray-300">{cardData.secondaryEquipment.armor.name}</span>
                </div>
              )}
              {cardData.secondaryEquipment.gadget && (
                <div className="flex items-center justify-between">
                  <span className="text-gray-400">Gadget:</span>
                  <span className="text-gray-300">{cardData.secondaryEquipment.gadget.name}</span>
                </div>
              )}
              {cardData.secondaryEquipment.communication && (
                <div className="flex items-center justify-between">
                  <span className="text-gray-400">Communication:</span>
                  <span className="text-gray-300">{cardData.secondaryEquipment.communication.name}</span>
                </div>
              )}
            </div>
          </div>
